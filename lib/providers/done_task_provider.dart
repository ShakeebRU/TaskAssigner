...
class DoneTaskProvider with ChangeNotifier {
  ...
  Future<bool> postTask(
      int taskId, int rating, String remarks, BuildContext context) async {
    UserModel? userData;
    await Preferences.init().then((value) {
      userData = value.getAuth();
    });
    ...
    final response = await http.post(
        Uri.parse(
            "${Utils.postTask}?taskid=$taskId&remarks=$remarks&ratingpoints=$rating"),
        headers: header);
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      print(jsonData);
